...
4. 如果用户反馈意味着**不要再做这个任务**（已超出范围 / 已被另一个任务替代），则走终态关闭路径——`project-feedback` 是 `close_task` 的主要调用方：
   ...
   - `cancelled` / `superseded` 是**终态**（无后继），只能经 `close_task` 进入；`update_task_status` 会拒绝这两个状态并要求改用 `close_task`；终态也不能再从 `done` 反向打开（v1 无 reopen）
...
- 区分"返工"与"不做"：返工 → `in_progress`（走 route-first workflow）；不做 → `close_task(cancelled|superseded)`；只有非终态任务可被关闭，已 `done` 的任务不可关闭/反向打开
